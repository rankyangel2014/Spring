...
public class Cryptos {
	...
	public static String aesEncrypt(String input) {
		try {
			byte[] key = Encodes.decodeHex(SaltManager.getEncryptionSalt());
			return Encodes.encodeHex(aesEncrypt(input.getBytes(DEFAULT_URL_ENCODING), key));
	...
	public static String aesDecrypt(String input) {
		try {
			byte[] key = StringUtils.isEmpty(SaltManager.getEncryptionSalt()) ? DEFAULT_KEY
					: Encodes.decodeHex(SaltManager.getEncryptionSalt());
			return new String(aesDecrypt(Encodes.decodeHex(input), key), DEFAULT_URL_ENCODING);
		} catch (UnsupportedEncodingException e) {
			return "";
		}
	}
