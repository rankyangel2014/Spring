...
public class WeedfsController {
	...
	public void view(HttpServletRequest req, HttpServletResponse resp, String fileName) throws Exception {
		...
		try {
			...
			logger.warn("Invalid If-Modified-Since header value: '" + req.getHeader("If-Modified-Since")
					+ "', ignoring");
		}
		...
		try {
			...
			// 附件的volumeId
			int volumeId = Integer.parseInt(filepath.substring(0, filepath.indexOf(COMMA)));
			final RestAdapter mainAdapter = new RestAdapter.Builder().setEndpoint(WeedfsUtil.weedServer.get(0)).build();
			...
			logger.info("file url " + volumeURL);
			String fileUrl = URL_PREFIX + volumeURL + URL_SPLIT + filepath + params;
