/*!
 * 扩展 Ext JS Library 3.4.0 groupGrid 表格并可以进行统计,统计包括小计和总计.
 */
Ext.ns('Ext.ux.grid');
appframe.loadCss("/ExtJs3.4/extend/GroupSummaryGrid.css","groupSummaryGridCss");
/**
    ...
    calculate : function(rs, cs){
        ...
        for(var j = 0, jlen = rs.length; j < jlen; j++){
            ...
            for(var i = 0, len = cs.length; i < len; i++){
                ...
                cf = cfg[i];
                if(cf.summaryType){
                    data[c.name] = Ext.ux.grid.GroupSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data,"subTotal");
                }
            }
        }
        return data;
...
Ext.ux.grid.GroupSummary.Calculations = {
    'sum' : function(v, record, field,data,type){
        return v + Ext.num(record.data[field], 0);
    },

    'count' : function(v, record, field, data,type){
        return data[field+'count'] ? ++data[field+'count'] : (data[field+'count'] = 1);
    },

    'max' : function(v, record, field, data,type){
        var v = record.data[field];
        var max = data[field+'max'] === undefined ? (data[field+'max'] = v) : data[field+'max'];
        return v > max ? (data[field+'max'] = v) : max;
    },

    'min' : function(v, record, field, data,type){
        var v = record.data[field];
        var min = data[field+'min'] === undefined ? (data[field+'min'] = v) : data[field+'min'];
        return v < min ? (data[field+'min'] = v) : min;
    },

    'average' : function(v, record, field, data,type){
        var c = data[field+'count'] ? ++data[field+'count'] : (data[field+'count'] = 1);
        var t = (data[field+'total'] = ((data[field+'total']||0) + (record.data[field]||0)));
        return t === 0 ? 0 : t / c;
    },
    'sign' : function(v, record, field, data,type){
    	if(type=="total"){
    		return "总计";
    ...
    for (var i = 0, len = cs.length; i < len; i++) {
      ...
      for (var j = 0, jlen = rs.length; j < jlen; j++) {
        ...
        if (cf && cf.summaryType) {
          data[c.name] = Ext.ux.grid.GroupSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data,"total");
        }
      }
    }
    return data;
...
com.jsjn.ext.extend.GroupSummaryGridPanel  =function(config){
	Ext.applyIf(this, config);
	this.initComponent();
	//com.jsjn.sample.grid.GroupWindow1.superclass.constructor.call(this);
}
