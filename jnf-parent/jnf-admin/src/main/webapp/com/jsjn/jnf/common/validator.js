...
/**
 * 组织机构代码合法性校验
 * @param  {[string]} val [组织机构代码]
 * @return {[boolean]}     [是否合规]
 */
function adjustOrgNoCommon(val) {
    var orgNo = val;
...
//						this.IDCardText = "身份证号码末位校验位校验出错！";
	...
	// 组织机构代码校验
    adjustOrgNo: function(val) {
        switch (val.length) {
            case 9:
            case 10:
                return adjustOrgNoCommon(val);
//            case 18:
...
//                    return false;
//                if(!adjustOrgNoCommon(charBetEigAndSeventeen))  //校验9到17位
//                    return false;
...
//                    return true;
    ...
    uniteCreditNoText: '请输入正确的统一信用代码',
	...
	bankNo : function(val, field) {
		// var reg =/^([0-9\-]\d)*$/;
		/** bug 7114 modify by liuaijun on 20141202 begin **/
		var reg = /^\d+([\-\－\-\－]{0,}\d{1,})+$/;
		/** bug 7114 modify by liuaijun on 20141202 end **/
		try {
			if (reg.test(val))
				return true;
			return false;
