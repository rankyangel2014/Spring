...
com.jsjn.jnf.trade.QryTradeListMgr = function(config) {
	if (typeof(com.jsjn.jnf.trade.QryTradeListMgr.PANEL) == 'undefined') {
		Ext.applyIf(this, config);
		this.initUIComponents();
		com.jsjn.jnf.trade.QryTradeListMgr.superclass.constructor.call(this);
		appframe
				.loadScripts(appframe.importJses['com.jsjn.jnf.trade.QryTradeListMgr']);
		com.jsjn.jnf.trade.QryTradeListMgr.PANEL = this;
	}
	return com.jsjn.jnf.trade.QryTradeListMgr.PANEL;
};
Ext.extend(com.jsjn.jnf.trade.QryTradeListMgr, Ext.Panel, {
	initUIComponents : function() {
		...
		this.store1068384417 = new Ext.data.Store({
			...
			url : appConfig.baseUrl
					+ '/jnf/tradeService.do?method=queryTransactionByCondition&qryFlag=0'
		});
		...
		this.gridPanel1436652731 = new Ext.grid.GridPanel({
			...
			columns : [{
				...
				renderer : function(value, cellmeta, record, rowIndex,
						columnIndex, store) {
					return com.jsjn.jnf.trade.QryTradeListMgr.gridRender(value,
		...
		this.dateField612596260 = new Ext.form.DateField({
			id : "modifiedMax.QryTradeListMgr",
			allowBlank : true,
			name : "modifiedMax",
			format : "Ymd",
			fieldLabel : "交易时间（止）",
		...
		this.textField1942793430 = new Ext.form.TextField({
			id : "externLoanNo.QryTradeListMgr",
			allowBlank : true,
			maxLength : 32,
			width : 200,
			name : "externLoanNo",
		...
		this.dateField751638090 = new Ext.form.DateField({
			id : "modifiedMin.QryTradeListMgr",
			allowBlank : true,
			name : "modifiedMin",
			format : "Ymd",
			fieldLabel : "交易时间（起）",
			...
			items : [{
				...
				buttons : [{
					...
					handler : function(button, event) {
						com.jsjn.jnf.trade.QryTradeListMgr.btnQuery(button,
								event);
					}
				}, {
					...
					text : "重置",
					handler : function(button, event) {
						com.jsjn.jnf.trade.QryTradeListMgr.btnReset(button,
								event);
					}
				}],
				collapsed : false,
