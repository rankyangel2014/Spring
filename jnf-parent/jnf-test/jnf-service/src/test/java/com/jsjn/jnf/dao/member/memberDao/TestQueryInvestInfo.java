 ...
 * queryInvestInfo测试思路：
 * 说明：根据mId查询某商户下投资人信息
 * 预置条件：数据库有1条记录（mId为商户1）
 * 1. 传入的mId为商户1，方法正常执行，返回1条记录
 * 2. 传入的mId为商户2，方法正常执行，无结果返回
 */
public class TestQueryInvestInfo {
	 ...
	 */
	@Test
	public void queryInvestInfo1(){
		Assert.assertNotNull(dao.queryInvestInfo(mid1));
	}
	 ...
	 */
	@Test
	public void queryInvestInfo2(){
		Assert.assertNull(dao.queryInvestInfo(mid2));
	}
