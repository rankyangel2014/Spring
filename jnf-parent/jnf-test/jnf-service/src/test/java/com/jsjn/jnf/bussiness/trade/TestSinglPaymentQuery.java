...
public class TestSinglPaymentQuery {
	 ...
	 * 测试的方法：
	 * singlePaymentQuery 测试思路：
	 * 说明:通过mid和tranNo联合查询数据,mid是由appkey产生,为空或不正确在protal层处理,此处不考虑
	 * 1. 业务请求中包含tranNo,tranNo为"",返回失败的resDto(ResCode为999999,
	 *    错误消息ResMsg为"", ResData为null)
	 * 2. 业务请求中包含tranNo,tranNo为null,返回失败的resDto(ResCode为999999,
	 *    错误消息ResMsg为"", ResData为null)
	 * 3. 根据mid和tranNo查询不到记录(mid不正确),返回失败的resDto(ResCode为001403,
	 *    错误消息ResMsg为"该支付订单号不存在！",ResData为null)
	 * 4. 根据mid和tranNo查询不到记录(tranNo不正确),返回失败的resDto(ResCode为001403,
	 *    错误消息ResMsg为"该支付订单号不存在！",ResData为null)
	 * 4. MD5校验失败,返回失败的resDto(ResCode为999999,
	 *    错误消息ResMsg为"该笔交易数据异常",ResData为null)
	 * 5. 所有检查正常，返回成功的resDto(ResCode为000000,
