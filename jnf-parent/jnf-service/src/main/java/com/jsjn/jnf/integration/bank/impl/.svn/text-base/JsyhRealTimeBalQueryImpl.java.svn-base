...
import com.jsjn.jnf.bean.bo.integration.jsyh.response.JsResRealtimeBalQueryBody;
...
@Service
@Transactional
public class JsyhRealTimeBalQueryImpl implements RealtimeBalQueryInterface {

	private final static Logger logger = Logger.getLogger(JsyhRealTimeBalQueryImpl.class);
	/**
	 * 报文流水
	 */
	...
	public RealtimeBalQueryResBO realTimeBalQuery(RealtimeBalQueryReqBO req) {
		 ...
		 * 1) 获取传参
		 */
		String bankInterfaceCode = BankInterfaceCode.JSYH_REALTIME_QUERY.getCode();
		RealtimeBalQueryReqDataBO reqData = req.getReqData();

		String accountNo = reqData.getAccountNo();
		String curCode = reqData.getCurCode();
		String tradeCode = BankInterfaceCode.JSYH_REALTIME_QUERY.getTrcode();//交易码
		...
		JsReqRealtimeBalQueryBody ctReqBody = (JsReqRealtimeBalQueryBody) ctReqDto.getBody();
		...
		ctReqHead.setReq_no(tradeCode + accountNo);//流水号
		...
		logger.info("开始调用江苏银行余额查询接口,请求报文信息：" + ctReqXML);
		...
		try {
			...
			logger.error("江苏银行余额查询接口调用异常", e);
			//接口调用异常，状态为：银行处理中。		
			resDto.setResCode(TradeCode.TRADE_DEAL.getCode());
			resDto.setResMsg(e.getMessage());
			// 更新报文流水表
			updateInterfaceXml(interfaceXmlSeq, ctResXML, "2", "江苏银行余额查询接口调用异常！,异常原因：" + e.getMessage());
			return resDto;
		}
		logger.info("结束调用江苏银行余额查询接口，返回结果信息：" + ctResXML);

		ctResDto = JaxbMapper.fromXml(ctResXML, JsResRealtimeBalQuery.class);
		JsResBaseHead ctResHead = ctResDto.getHead();
		JsResRealtimeBalQueryBody ctResBody = (JsResRealtimeBalQueryBody) ctResDto.getBody();
		/**
		 * 4) 转化返回对象
		 */
		/**
		...
		if (!StringUtils.equals(succFlag, "0")) {//0-表示接口调用成功
			...
			resDataDto.setAccountName(ctResBody.getAcname());
			resDataDto.setAccountNo(ctResBody.getAcno());
			resDataDto.setCurCode(ctResBody.getCur_code());
			resDataDto.setUseBalance(ctResBody.getUse_balance());
	...
	public void insertInterfaceXml(String id, String inputXml) {
		...
		dto.setId(id);
		dto.setMethod("JsyhRealTimeBalQueryImpl.realTimeBalQuery");
