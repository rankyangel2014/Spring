...
//    	System.out.println(time);
     ...
     * 		不在		  : false
     */
    public static boolean timeInRange(String srcDt ,String dstDt , boolean nextDay){
    	int srcHour = Integer.parseInt(srcDt.split(":")[0]);
    	int srcMinute = Integer.parseInt(srcDt.split(":")[1]);
    	
    	int dstHour = Integer.parseInt(dstDt.split(":")[0]);
    	int dstMinute = Integer.parseInt(dstDt.split(":")[1]);
    	
    	Calendar currCal = Calendar.getInstance();
    	Calendar srcCal = Calendar.getInstance();
    	Calendar dstCal = Calendar.getInstance();
    	
    	srcCal.set(Calendar.HOUR_OF_DAY, srcHour);
    	srcCal.set(Calendar.MINUTE, srcMinute);
    	srcCal.set(Calendar.SECOND, 0);
    	
    	dstCal.set(Calendar.HOUR_OF_DAY, dstHour);
    	dstCal.set(Calendar.MINUTE, dstMinute);
    	dstCal.set(Calendar.SECOND, 0);
    	
    	if(nextDay){
    		dstCal.add(Calendar.DATE, 1);
    	}
    	
    	//如果在规定的事件范围内，返回true，否则返回false
    	if(currCal.compareTo(srcCal) >= 0 && currCal.compareTo(dstCal) <= 0)
    		return true;
